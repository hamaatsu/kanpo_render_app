...
def stage2_llm_selection(form: Dict[str, Any], pool: list, sex: str) -> Dict[str, Any]:
    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        return simple_assess_with_pool(form, pool)
    try:
        from openai import OpenAI
        client = OpenAI(api_key=api_key)
        sys_prompt = (
        ...
        ass = simple_assess_with_pool(form, pool)
...
def complaint_rerank(assessment: dict, form: dict) -> dict:
    ...
    try:
        ...
        if dom == "dermatology":
            ...
            if isinstance(lifestyle, str): lifestyle = [lifestyle]
            ls = list({*lifestyle, "辛味・油の摂り過ぎを控える", "十分な睡眠", "肌への強い刺激を避ける"})
...
def simple_assess_with_pool(form: Dict[str, Any], pool: list) -> Dict[str, Any]:
    ass = simple_assess(form)
