...
4) 処方スコアリング（内部想定の候補群に対し0〜1で算出）：
   ...
   - 選ばれた各候補には必ず "priority_basis" キーを付け、「症状優先」「証優先」「折衷」のいずれかを記載する。
...
8) ツボのアドバイス（80〜100文字）。
   主症状と証を踏まえ、日本で一般的な経穴名と位置・押し方を含めて段落で記述。
9) アロマのアドバイス（80〜100文字）。
   証と主症状に合う精油（精油名は必ず3種類）を提案。
    それぞれ香りの特徴と使用方法（ディフューズ、アロマバスなど）を含め、1つの段落として記述。
